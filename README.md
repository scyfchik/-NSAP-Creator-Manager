...
This is a full-stack application: Express serves the API, Discord OAuth, PostgreSQL production persistence, JSON backups, audit logs, and the existing frontend UI. SQLite remains available for local development only.
...
DATABASE_SSL=true
SQLITE_PATH=./server/data/nsap.sqlite
PORT=4173
...
In production, `DATABASE_URL` must be a PostgreSQL URL; startup fails instead of silently using SQLite. `SESSION_SECRET` and all Discord OAuth variables are also required. Cookies are HTTP-only and marked Secure when `NODE_ENV=production`.
...
PostgreSQL is required in production. Tracked, idempotent migrations run at startup and via `npm run migrate`. Local development uses `SQLITE_PATH` when `DATABASE_URL` is empty.

Import a legacy SQLite database without deleting it:

```bash
DATABASE_URL=postgresql://... SQLITE_PATH=./server/data/nsap.sqlite npm run migrate:sqlite-to-postgres
```

The importer safely skips existing rows and imports creators, users/roles, embedded timelines, and audit logs. It uses `data/creators.json` when SQLite has no creators.
...
`data/creators.json` seeds an empty database only. Runtime dashboard data always comes from the selected database adapter.
...
Render must attach the managed PostgreSQL internal connection string as `DATABASE_URL` and set `DATABASE_SSL=true`. The service does not need a persistent disk. Before the first PostgreSQL deployment, run `npm run migrate:sqlite-to-postgres` with both `DATABASE_URL` and `SQLITE_PATH` set when legacy SQLite data must be retained.

After deployment, verify persistence by editing a creator and restarting the service. Creator fields, quick notes, follow-up dates, timelines, role assignments, sessions, and audit logs must remain present.

`render.yaml` defines a Node web service and managed PostgreSQL database; no persistent disk is required.
...
`SESSION_SECRET` can be generated by Render. `DATABASE_URL` is populated from the managed PostgreSQL database in `render.yaml`.
...
- `server/db/` - shared database interface, PostgreSQL pool, SQLite fallback, and migrations
...
The backend uses Helmet, compression, CORS, rate limiting, CSRF protection, HTTP-only sessions, secure production cookies, request validation, parameterized database queries, and a global error handler.
